...
from processing_modules.pdfextract import pdftext,insert_pdf_data
from processing_modules.adharcard_long import adhar, QRcode
from processing_modules.image_to_text import insert_img_data, simple_image_extract
from processing_modules.QRreader import Decode, insert_qr_data
import pandas as pd
...
def upload_image():
    ...
    if ch=='Adhar card':
        ...
        if len(text)==0 or text == '':
            text=QRcode(fpath)
        try:
            check = request.form['img_store']

            if check:
                if 'user_id' in session:
                    user_id = session['user_id']
                    insert_img_data(text,user_id)
        except:
            pass
        return render_template('imgtext.html',img_path = other_page,impath=fpath,extext=text)
    elif ch!='Adhar card' and ch!=None:
        f.save(os.path.join('static','plainimg',fname))
        fpath=os.path.join('static','plainimg',fname)
        text=simple_image_extract(fpath)
        return render_template('imgtext.html',img_path = other_page,impath=fpath,extext=text)
...
def pdf():
    if request.method=='POST':
        ...
        try:
            check = request.form['pdf_store']

            if check:
                if 'user_id' in session:
                    user_id = session['user_id']
                    insert_pdf_data(text,user_id)
        except:
            pass
